...
b)  Métodos de acesso (getter/setter) dos atributos do ponto por você identificado;
c)	Métodos sobrecarregados de movimentação do ponto com os mesmos parâmetros indicados para os construtores, ou seja, o ponto “sai” de uma localização e “vai” para a localização nova;
d)   Método que permita calcular a distância do ponto que recebe a mensagem, para um outro ponto (objeto) informado;
e)  Método que permita a criação de um novo ponto no mesmo local do ponto que recebeu a mensagem (clonar).
...
e)  Um método imprima uma representação do número complexo, ou seja: a + ib, onde a é a parte real e b é a parte imaginária
...
b)  Método de acesso (getter) para:
●      raio do círculo (double)
●      centro do círculo (texto)
c)  Métodos inflar e desinflar, que, respectivamente, aumentam e diminuem o raio do círculo de um valor informado
●      Obs.: o centro do círculo permanece inalterado
d)	Métodos sobrecarregados para inflar e desinflar, que respectivamente, aumentam e diminuem o raio do círculo de uma unidade
●      Obs.: o centro do círculo permanece inalterado
e)  Métodos sobrecarregados para mover o círculo, que:
